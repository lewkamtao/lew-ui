...
import type { MenuTreeOptions } from './props'
...
const generateEnterpriseMenu = (menuData: MenuTreeOptions[], level = 1): MenuTreeOptions[] => {
  return menuData.map((item) => {
    const { children } = item
    const menuItem: MenuTreeOptions = {
      ...item,
      level // 添加 level 属性
    }
...
const _options = ref<MenuTreeOptions[]>([])
