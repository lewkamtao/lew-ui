...
/** 边框、留白等区域未命中 input 时委托聚焦；capture 避免被 tooltip 等 stopPropagation */
function handleInputNumberViewMouseDown(e: MouseEvent) {
  ...
  if (!el)
    return
  if (el.closest('.lew-input-number-control'))
    return
  const inputEl = lewInputRef.value
  if (!inputEl || inputEl.disabled)
    return
  if (inputEl === el || inputEl.contains(el))
    return
...
<template>
  <div
    ...
    @click="handleInputNumberViewMouseDown"
  >
    <input
      ref="lewInputRef"
      v-model="modelValue"
