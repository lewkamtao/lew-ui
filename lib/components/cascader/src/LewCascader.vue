...
function findAndAddChildrenByValue(
    ...
    for (const node of tree) {
        ...
        if (node.children && node.children.length > 0) {
            const result = findAndAddChildrenByValue(
                node.children,
                value,
                children,
            )
            ...
            ((props.options &&
                props.options.map((e: CascaderOptions) => {
                    return {
                        ...
                        isLeaf:
                            !e.children ||
                            (e.children && e.children.length === 0), // 没有孩子
                    }
                })) as CascaderOptions[]) || []
    }
                        ...
                        >
                            <div
                                ...
                                :style="{
                                    ...
                                    transform:
                                        oItem.length > 0
                                            ? `translateX(${200 * oIndex}px)`
                                            : '',
                                }"
                            >
                                <virt-list
                                    :key="
                                        oItem[0]?.parentValuePaths?.join('-') ||
                                        'root'
                                    "
                                    :ref="
                                ...
                                >
                                    <template
                                        #default="{ itemData: templateProps }"
                                    >
                                        <div
