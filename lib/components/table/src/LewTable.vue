...
onMounted(() => {
    ...
    if (
        props.columns.some(
            (col: any) =>
                !col.width && (!col.children || col.children.length === 0),
        )
    ) {
...
const updateDragTarget = (mouseY: number) => {
    ...
    if (!targetRowId && positionEntries.length > 0) {
        if (mouseY < positionEntries[0].top) {
            ...
            mouseY > positionEntries[positionEntries.length - 1].bottom
        ) {
...
const getFixedColumns = computed(() => (direction: string) => {
    return (
        fixedColumns.value[direction as keyof typeof fixedColumns.value] || []
    )
})
...
<template>
    <div ref="tableWrapperRef" class="lew-table-wrapper">
        ...
        <div
            class="lew-table-drag-indicator"
        ...
        <div
            ...
            :class="{
                ...
                'lew-table-has-fixed-right':
                    getFixedColumns('right').length > 0,
            }"
        ...
        >
            <div
                ...
                :style="{
                    ...
                    height:
                        getHeadHeight * columnLevel + columnLevel * 1 + 'px',
                }"
                @mouseenter="state.hoverRowIndex = -1"
            >
                <div
                    v-if="
                        getFixedHeaderColumns('left').length > 0 ||
                        checkable ||
                        sortable
                    "
                    ref="fixedLeftRef"
                    ...
                    <div class="lew-table-tr">
                        ...
                        >
                            <lew-checkbox
                                ...
                                :certain="
                                    hasPartialSelection && !state.isAllChecked
                                "
                        ...
                        <readerHeaderTd
                            :column="column"
                            v-for="(column, index) in getFixedHeaderColumns(
                                'left',
                            )"
                            :key="`columns${index}`"
                    ...
                    <div
                        ...
                        :class="{
                            ...
                            'lew-table-tr-selected':
                                state.selectedRowsMap[row[rowKey]],
                        }"
                        :style="{
