...
export default defineComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    async function beforeUpload(file: UploadRawFile) {
      ...
      if (props.autoUpload) upload(proxyUploadFile)
    }
          ...
          {props.fileList.map((file) => {
            return (
              <li class="lew-upload-list__item">
                <div class="lew-upload-list__item-info" onClick={() => handlePreview(file)}>
                  {renderIcon({
                    isRender: true,
                    type: 'file-text',
                    size: 14
                ...
                <div class="lew-upload-list__item-status" v-show={file.status === 'success'}>
                  {renderIcon({
                    isRender: true,
                    type: 'check-circle',
                    size: 14,
