...
export const titleProps = {
  ...
  color: {
    ...
    validator: (value: string) => {
      ...
      if (value && !validColors.includes(value as TitleColor) && !/^#([0-9A-Fa-f]{3}){1,2}$/.test(value)) {
  ...
  bold: {
    ...
    validator: (value: number) => {
      if (!([100, 200, 300, 400, 500, 600, 700, 800, 900] as BoldType[]).includes(value as BoldType)) {
