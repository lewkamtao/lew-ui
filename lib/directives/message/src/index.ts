...
const setMessage = ({ type, e }: MessageOptions) => {
    ...
    messageDom.setAttribute('class', `message message-${type} message-id-${e.id}`);
    setTimeout(() => {
        messageDom.setAttribute('class', `message message-${type} message-show`);
        timer[e.id] = setTimeout(() => {
            messageDom.setAttribute('class', `message message-${type} message-hidden`);
            setTimeout(() => {
                try {
                    if (messageDom) LewMessageDom?.removeChild(messageDom);
                } catch {}
