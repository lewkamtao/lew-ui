import type { LewBackTopDirectiveBinding } from 'lew-ui/types'
...
export const LewVBackTop = {
  install(app: App) {
    app.directive('backtop', {
      mounted(el: ExtendedHTMLElement, binding: DirectiveBinding<LewBackTopDirectiveBinding>) {
        const backTopHandler = createBackTopHandler(binding.value.target)
