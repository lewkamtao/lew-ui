...
export default {
  ...
  components: {
    ...
    textTrim: {
      ...
      props: {
        text: '文本内容；与默认插槽二选一，优先使用 text',
        lineClamp: '文本显示的最大行数，超出部分将被截断；与 reserveEnd 同时使用时以多行截断为准',
        allowHTML: '提示框内容是否按 HTML 解析（仅在使用 text 时生效；插槽内容为 DOM 节点不受此影响）',
        placement: '提示框的显示位置',
        offset: '提示框相对于目标元素的偏移量 [x, y]（单位：像素）',
        textAlign: 'x轴对齐方向（当宽度小于容器宽度时）',
        delay: '显示和隐藏提示框的延迟时间 [显示延迟, 隐藏延迟]（单位：毫秒）',
        reserveEnd:
          '保留末尾的字符数量，当文本溢出时会在中间显示省略号，保留开头和指定数量的末尾字符（仅单行且使用 text 时生效）',
      },
    },
    flex: {
      name: '弹性布局 Flex',
    ...
    table: {
      ...
      demo13: {
        title: '虚拟滚动',
        description:
          '10 万行开启 ```virtual```，配合左右固定列与多选。虚拟滚动下行高固定；关闭虚拟滚动后行高仍随内容自适应。',
      },
